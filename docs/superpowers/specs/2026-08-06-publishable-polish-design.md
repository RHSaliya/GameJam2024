...
| `EndScene` | Star rating vs par time, per-mission best comparison, drawn icons, and a `watchResponsiveLayout` listener — every element is currently hardcoded to x=640 with no listener, so it alone among the high-traffic scenes never adapts to tablet or ultrawide viewports. The same listener is added to `InstructionsScene`, `AchievementsScene`, and `Credits`, which share the gap. |
